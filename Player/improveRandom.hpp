    ...
    virtual std::string decideHand(std::string_view res){
        ...
        for(const Unit& u: units){
            if(u.color() == UnitColor::Blue){
                ...
                if(u.y() > 1 && upt < 20){
                    //if(game.canMove1st(u, Direction::North))
        ...
        /*if(distp[0]<diste[0]){
            for(const Unit& u: units){
                if(u.color() == UnitColor::Blue){
                    if(distp[0] == u.x() + u.y()){
                        //if(u.x() != 0 && game.canMove1st(u, Direction::West))return Hand{u, Direction::West};
                        //else if(game.canMove1st(u, Direction::North))return Hand{u, Direction::North};
                    }
                }
            }
        }
            ...
            for(const Unit& u: units) {
                if(u.color() == UnitColor::Blue) {
                    if (distp[1] == 5 - u.x() + u.y()) {
                        //if (u.x() != 5&&game.canMove1st(u, Direction::East))return Hand{u, Direction::East};
                        //else if(game.canMove1st(u, Direction::North))return Hand{u, Direction::North};
                    }
                }
            }
...
#pragma omp parallel for
        for(int i=0;i<legalMoves.size();i++){
            ...
            if(ras>mont){monthand=i;mont=ras;}
